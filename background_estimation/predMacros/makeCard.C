...
void addSDMassSysts(DataCardMaker& card, std::string yr) {
    if(yr == "") {
        ...
        card.addParamSystematic("hbb_res"+yr,    1.0-1.0,0.20);
        card.addParamSystematic("hbb_res_top"+yr,1.0-1.0,0.20);
    } else if(yr == "_17") {
    	card.addParamSystematic("hbb_scale"+yr,    0.982-1.0,0.004);
        card.addParamSystematic("hbb_scale_top"+yr,0.982-1.0,0.01*0.982);
        card.addParamSystematic("hbb_res"+yr,    1.092-1.0,0.05);
        card.addParamSystematic("hbb_res_top"+yr,1.092-1.0,0.05);
    } else if(yr == "_18") {
    	card.addParamSystematic("hbb_scale"+yr,    0.997-1.0,0.004);
        card.addParamSystematic("hbb_scale_top"+yr,0.997-1.0,0.01*0.997);
        card.addParamSystematic("hbb_res"+yr,    1.243-1.0,0.041);
        card.addParamSystematic("hbb_res_top"+yr,1.243-1.0,0.041);
    }
...
void addSignalNormSysts(DataCardMaker& card, std::string sigName, bool is1l, TString fullcat) {
    ...
    if(is1l) {
        ...
        // lepton
    	if(isEorSF) {
            card.addSystematic("e_trigger","lnN",{{sigName,1.02}});   // trigger measurement
            ...
            card.addSystematic("mu_trigger","lnN",{{sigName,1.02}});   // trigger measurement
    	...
    	// tau21
    	if(isLP) {
    		card.addSystematic("tau21_eff","lnN",{{sigName,tau21LPFrac*(1-unc_tau21_LP_Run2)+(1-tau21LPFrac)*(1+unc_tau21_HP_Run2)}});
    	} else {
        ...
        // lepton
    	if(isEorSF) {
            card.addSystematic("ee_trigger","lnN",{{sigName,1.0+(0.03*eeFrac)}});   // trigger measurement
            card.addSystematic("mm_trigger","lnN",{{sigName,1.0+(0.03*mmFrac)}});   // trigger measurement

    	    card.addSystematic("e_Reco" ,"lnN",{{sigName,1.0+(0.008*eeFrac)}});     // measurement -> eReco weights * ee fraction
    	    card.addSystematic("e_ID_2l" ,"lnN",{{sigName,1.0+(0.027*eeFrac)}});    // measurement -> eID weights * ee fraction
    	    card.addSystematic("e_ISO" ,"lnN",{{sigName,1.0+(0.06*eeFrac)}});       // assertion in AN * ee fraction
    	    card.addSystematic("mu_ID_2l" ,"lnN",{{sigName,1.0+(0.025*mmFrac)}});   // measurement -> muID weights * mumu fraction
    	    card.addSystematic("mu_ISO" ,"lnN",{{sigName,1.0+(0.04*mmFrac)}});      // assertion in AN * mumu fraction
    	} else {
            card.addSystematic("em_trigger","lnN",{{sigName,1.0+(0.03*0.5)}});   // trigger measurement
            card.addSystematic("me_trigger","lnN",{{sigName,1.0+(0.03*0.5)}});   // trigger measurement

    	    card.addSystematic("e_Reco" ,"lnN",{{sigName,1.008}});    // measurement -> eReco weights
    	    card.addSystematic("e_ID_2l" ,"lnN",{{sigName,1.026}});   // measurement -> eID weights
    ...
    // AK8 b jets
    if(isLoose) {
        card.addSystematic("bAk8_eff","lnN",{{sigName,1.085}});  // DeepAK8-MD Z/H->bb weights
    } else {
        card.addSystematic("bAk8_eff","lnN",{{sigName,1.115}});  // DeepAK8-MD Z/H->bb weights
    }
...
std::vector<std::string> years {"_16","_17","_18"};
if(doCombinedRun2) years = {""};

for(const auto& yr : years) {

sigInputDir = mainDir + "/signalInputs"+yr+"/";
sfPF = sigInputDir + filename;
switch(reg){
case REG_SR:
    fPF = mainDir + "/bkgInputs"+yr+"/" + filename;
    break;
case REG_TOPCR:
    fPF = mainDir + "/bkgInputsTopCR"+yr+"/" + filename + "_TopCR";
    break;
case REG_NONTOPCR:
    fPF = mainDir + "/bkgInputsNonTopCR"+yr+"/" + filename + "_NonTopCR";
    break;
}
...
if(channel == 0 || channel == 1) {
    for(const auto& l :lepCats) for(const auto& b :btagCats) for(const auto& p :purCats) for(const auto& h :selCuts1){
        ...
        auto systName = [&](const std::string& proc,const std::string& name, const std::string& sel = "-1")->std::string {
            return getSystName("",proc,name, sel == "-1" ? l +"_"+b+"_"+p+yr : sel+yr  );
        };
...
//        card.rebinY(hhMassRebins);
        ...
        double rate_mt = 0;
        if(reg != REG_NONTOPCR) {
        	rate_mt = card.addFixedYieldFromFile(bkgSels[BKG_MT],4,fPF+"_"+bkgSels[BKG_MT]+"_distributions.root",bkgSels[BKG_MT]+"_"+cat+"_"+hhMCS,1.0,true);
        }
        //---------------------------------------------------------------------------------------------------
        ...
        if(reg != REG_NONTOPCR) card.addParamSystematic(systName("top","mt_rel_scale",b+"_1l") ,0.0,0.50);
...
//            card.addSystematic(systName("top","tFrac",b+"_1l")         ,"lnN",{{bkgSels[BKG_MT],1.0+0.30},{bkgSels[BKG_MW],fabs(1.0-0.30*rate_mt/rate_mw)}});
        ...
        card.add2DSignalParametricShape(signalName,MOD_MJ,MOD_MR, signalInputName(signalName,"2D_fit.json"),
					{{"hbb_scale"+yr,1}},{{"hbb_res"+yr,1}},sigHHScaleSysts,sigHHResSysts, b == btagCats[BTAG_L],MOD_MS);
...
if (channel == 0 || channel == 2) {
    for(const auto& l :dilepCats) for(const auto& b :btagCats) for(const auto& s :selCuts2){
        ...
        auto systName = [&](const std::string& proc,const std::string& name, const std::string& sel = "-1")->std::string {
            return getSystName("",proc,name, sel == "-1" ? l +"_"+b+yr : sel+yr  );
        };
...
//        card.rebinY(hhMassRebins);
        ...
        if(reg != REG_NONTOPCR) card.addParamSystematic(systName("top","mt_rel_scale",b+"_2l") ,0.0,0.75);
